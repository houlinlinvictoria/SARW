...
#                        feats = np.load('feats.npy')  #
                        ...
                        for i in range(0, deg_current_node-1):
                            for j in range(0, deg_next_node-1):
                                x_i = self.class_arr[current_node_nbhd[i]]  #当前节点的某个邻居的feature
                                x_j = self.class_arr[next_node_nbhd[j]]  #下一节点的某个邻居的feature
                                x_i_norm = np.linalg.norm(x_i)
                                x_j_norm = np.linalg.norm(x_j)
                                cos = np.dot(x_i, x_j)/(x_i_norm * x_j_norm)
                                #sim = cos
                                sim_sum = sim_sum + cos
                        SIM = sim_sum/min(deg_current_node,deg_next_node)
                        alfa=1   #alfa=1 时为原来cnarw
