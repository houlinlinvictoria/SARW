...
class Minibatch:
    ...
    def __init__(self, adj_full_norm, adj_train, role, class_arr, placeholders, train_params, **kwargs):
        ...
        assert self.class_arr.shape[0] == self.adj_full_norm.shape[0] #assert 断言：不满足条件时直接返回false
...
#设定采样方法
    def set_sampler(self,train_phases):
        ...
        #pure python demo
        elif self.method_sample == 'CNARW_SamplingVanillaPython':
            ...
            self.graph_sampler = SARW_SamplingVanillaPython(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']))  
        elif self.method_sample == 'SARW_nofeatureSamplingVanillaPython':   #SARW without feature, with alph.
            self.size_subg_budget = train_phases['num_root']*train_phases['depth']
            self.graph_sampler = SARW_nofeatureSamplingVanillaPython(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']))  
        elif self.method_sample == 'NodeSamplingVanillaPython':
            self.size_subg_budget = 300
            self.graph_sampler = NodeSamplingVanillaPython(self.adj_train,\
                self.node_train,self.size_subg_budget)
        elif self.method_sample == 'rw':
            self.size_subg_budget = train_phases['num_root']*train_phases['depth']
            self.graph_sampler = rw_sampling(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']))
        elif self.method_sample == 'RW_SamplingVanillaPython':    #增加的rw python
            self.size_subg_budget = train_phases['num_root']*train_phases['depth']
            self.graph_sampler = RW_SamplingVanillaPython(self.adj_train,\
                self.node_train,self.size_subg_budget,int(train_phases['num_root']),int(train_phases['depth']))
        ...
        for i in range(num_subg):
            if self.subgraphs_remaining_edge_index[i] != []:
            #print(self.subgraphs_remaining_edge_index[i])
    ...
    def feed_dict(self,mode='train'):
        ...
        if mode in ['val','test']:
            ...
            #modify2: if use pure python: need to transfer
            adj.data = adj.data.astype(np.float32)
            adj_edge_index = adj_edge_index.astype(np.int32)
